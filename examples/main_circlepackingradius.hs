...
fnewcontent nodes context = newhue 
	    where 
	        newhue   = maxhue + (sample (symRange 0.1) ((take (newindex + 1) randgenhues) !! newindex))
		randgenhues = fst (contextcontent context)
...
fnewradius nodes context =  newradius
	    where 
	          newradius     = maxradius * (1.0 + (sample (symRange 0.1) ((take (newindex + 1) randgenradius) !! newindex)))
		  randgenradius = snd (contextcontent context)
...
main = do
	 ...
	 randgenhue       = randomRs (0.0,1.0) (mkStdGen 0) :: [Float]
